/*-------------------------------------------------------------------------
 *
 * Daniel Foreign Data Wrapper for PostgreSQL
 *
...
typedef struct FileFdwExecutionState
{
    ...
    bool      emptynull;                /* required for spcial empty line case */
} FileFdwExecutionState;

/*
 * SQL functions
 */
extern Datum min_fdw_handler(PG_FUNCTION_ARGS);
extern Datum min_fdw_validator(PG_FUNCTION_ARGS);

PG_FUNCTION_INFO_V1(min_fdw_handler);
PG_FUNCTION_INFO_V1(min_fdw_validator);

/*
 * FDW callback routines
 */
static void minGetForeignRelSize(
    PlannerInfo *root, 
    RelOptInfo *baserel,
    Oid foreigntableid
);
static void minGetForeignPaths(
    PlannerInfo *root,
    RelOptInfo *baserel,
    Oid foreigntableid
);
static ForeignScan *minGetForeignPlan(
    PlannerInfo *root,
    RelOptInfo *baserel,
    Oid foreigntableid,
...
static void minBeginForeignScan(
    ForeignScanState *node, 
    int eflags
);
static TupleTableSlot *minIterateForeignScan(
    ForeignScanState *node
);
static void minReScanForeignScan(
    ForeignScanState *node
);
static void minEndForeignScan(
    ForeignScanState *node
);
static void minExplainForeignScan(
    ForeignScanState *node, 
    ExplainState *es
);
static bool minAnalyzeForeignTable(
    Relation relation,
    AcquireSampleRowsFunc *func,
    BlockNumber *totalpages
);
static List * minImportForeignSchema(
...
static void get_table_name(char *filename, char **tbl);
static void get_fields_stmt(char *filename, StringInfoData *fields_stmt);
/*
 * Foreign-data wrapper handler function: return a struct with pointers
 * to my callback routines.
 */
Datum min_fdw_handler(PG_FUNCTION_ARGS) {
    FdwRoutine *fdwroutine = makeNode(FdwRoutine);

    fprintf(stderr, "%s\n", "Daniel_MIN_FDW: In fdw_handler.\n");
    fdwroutine->GetForeignRelSize = minGetForeignRelSize;
    fdwroutine->GetForeignPaths = minGetForeignPaths;
    fdwroutine->GetForeignPlan = minGetForeignPlan;
    fdwroutine->BeginForeignScan = minBeginForeignScan;
    fdwroutine->IterateForeignScan = minIterateForeignScan;
    fdwroutine->ReScanForeignScan = minReScanForeignScan;
    fdwroutine->EndForeignScan = minEndForeignScan;
    fdwroutine->AnalyzeForeignTable = minAnalyzeForeignTable;	
    fdwroutine->ExplainForeignScan = minExplainForeignScan;
    
    /* Support functions for IMPORT FOREIGN SCHEMA */
    fdwroutine->ImportForeignSchema = minImportForeignSchema;
 ...
 */
Datum min_fdw_validator(PG_FUNCTION_ARGS) 
{
    ...
    fprintf(stderr, "%s\n", "Daniel_MIN_FDW: In fdw_validator.\n");
    /*
...
/*
 * obtion relation size estimaters for a foreign table.
 */
static void minGetForeignRelSize(PlannerInfo *root, RelOptInfo *baserel, Oid foreigntableid){
    FileFdwPlanState *fdw_private;

    fprintf(stderr, "%s\n", "Daniel_MIN_FDW: In GetForeignRelSize.\n");
    /*
...
static void minGetForeignPaths(PlannerInfo *root, RelOptInfo *baserel, Oid foreigntableid) {
    FileFdwPlanState *fdw_private = (FileFdwPlanState *) baserel->fdw_private;
    Cost startup_cost;
    Cost total_cost;

    fprintf(stderr, "%s\n", "Daniel_MIN_FDW: In GetForeignPaths.\n");
    /* Estimate costs */
...
static ForeignScan *minGetForeignPlan (PlannerInfo *root, RelOptInfo *baserel, Oid foreigntableid, ForeignPath *best_path, List *tlist, List *scan_clauses, Plan *outer_plan){
    Index scan_relid = baserel->relid;

    fprintf(stderr, "%s\n", "Daniel_MIN_FDW: In GetForeignPlan.\n");
    /*
 ...
 */
static void minBeginForeignScan(ForeignScanState *node, int eflags){
    char *filename;
    List *options;
    CopyState cstate;
    ...
    fprintf(stderr, "%s\n", "Daniel_MIN_FDW: In BeginForeignScan.\n");
    /*
 ...
 minIterateForeignScan(ForeignScanState *node){
    FileFdwExecutionState *festate = (FileFdwExecutionState *) node->fdw_state;
    TupleTableSlot        *slot = node->ss.ss_ScanTupleSlot;
    bool                  found;
    ErrorContextCallback  err_context;
        
    fprintf(stderr, "%s\n", "Daniel_MIN_FDW: In IterateForeignScan.\n");
...
static void
minEndForeignScan(ForeignScanState *node){
    FileFdwExecutionState *festate = (FileFdwExecutionState *) node->fdw_state;

    fprintf(stderr, "%s\n", "Daniel_MIN_FDW: In EndForeignScan.\n");
    /* if festate is NULL, we are in EXPLAIN; nothing to do */
    if (festate) EndCopyFrom(festate->cstate);
}

static void
minReScanForeignScan(ForeignScanState *node){
    FileFdwExecutionState *festate = (FileFdwExecutionState *) node->fdw_state;
...
/*
 * minAnalyzeForeignTable
 * Test whether analyzing this foreign table is supported
 */
static bool
minAnalyzeForeignTable(Relation relation, AcquireSampleRowsFunc *func,
                       BlockNumber *totalpages)
{
    char *filename;
    List *options;
...
/*
 * minExplainForeignScan
 *		Produce extra output for EXPLAIN
 */
static void
minExplainForeignScan(ForeignScanState *node, ExplainState *es)
{
    char *filename;
    List *options;

    fprintf(stderr, "%s\n", "Daniel_MIN_FDW: In ExplainForeignScan.\n");
 ...
 */
static List *
minImportForeignSchema(ImportForeignSchemaStmt *stmt, Oid serverOid)
{
    List *commands = NIL;
    ForeignServer *server;
    ListCell *lc;
    StringInfoData buf;
    StringInfoData fields_stmt;
    char *filename = NULL;
    char *format = NULL;
    char *has_header = NULL;
    char *tbl = NULL;

    fprintf(stderr, "%s\n", "Daniel_MIN_FDW: In ImportForeignSchema.\n");
    ...
    get_fields_stmt(filename, &fields_stmt);
    get_table_name(filename, &tbl);

    initStringInfo(&buf);
    appendStringInfo(
        &buf, 
       "CREATE FOREIGN TABLE %s(%s) SERVER %s "
       "OPTIONS (filename '%s', format '%s', header '%s');", 
        tbl, pstrdup(fields_stmt.data), server->servername, 
        filename, format, has_header
    );
    commands = lappend(commands, pstrdup(buf.data));
    
//    resetStringInfo(&buf);
//    appendStringInfoString(&buf, "CREATE FOREIGN TABLE b (id int, remarks text) SERVER minserver OPTIONS (filename '/home/vagrant/project/daniel_fdw/sample_data.csv', format 'csv');");
//    commands = lappend(commands, pstrdup(buf.data));
    return commands;
}
...
static void get_table_name(char *path, char **tbl) {
    char *path_c = (char *)palloc(sizeof(path));
    char *file_basename = NULL;

    strcpy(path_c, path);
    file_basename = basename(path_c);
    *tbl = strsep(&file_basename, ".");
}

static void get_fields_stmt(char *path, StringInfoData *fields_stmt) {
    ...
    /* Read the first line */
    if((read_num = getline(&header, &len, fileStream)) == -1) {
        fprintf(stderr, "File '%s' has no content.\n", path);
    }

    /* Split into tokens and resemble into statement */
    initStringInfo(fields_stmt);
    field = strsep(&header, ",\n");
    for(pad = ""; field != NULL && strcmp(field, ""); field = strsep(&header, ",\n")) {
        column   = strsep(&field, ":");
        datatype = strsep(&field, ":");
        appendStringInfo(fields_stmt, "%s%s %s", pad, column, datatype);
        pad = ", ";
    }
